...
```
*Note*: If install from Github, additional packages might need to be installed before the assignPOP can be successfully installed. Follow the hints that R provided and re-run `install_github("alexkychen/assignPOP")`.
...
The package provides two resampling cross-validation methods: Monte-Carlo cross-validation for evaluting assignment accuracy; *K*-fold cross-validation for estimating membership probability.
...
Third, perform an assignment test using a desired predicitive model, for example, naive Bayes.
```{r, eval=FALSE}
