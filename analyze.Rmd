...
For the GENEPOP format, data should be saved with only one group - only one 'pop' label in the file. For the STRUCTURE format, the column of population label should also be included, but the label can be anything (this column will be ignored when performing the assignment test).   
...
strin_unknown <- read.Structure( "simStructureX.txt" )
```
...
The non-genetic data of unknown individuals (**OtherFeatures.csv**) should also have the identical sample IDs as the genetic data (**simGenepopX.txt**) have. The sample IDs should also be saved in the first column, and there would be no population label in the last column of your non-genetic data.
...
After importing your baseline data (training) and unknown individuals (test), the assignment test can be performed using the function `assign.X`. Make sure your training and test data have the same data type and feature names. Use the argument `X1` to specify the training data; `X2`, the test data.
